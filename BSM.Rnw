...
Gamma ($\Gamma$) is the second derivative of the option pricing function with respect to the underlying price, time been constant (\ref{eq:GammaDerivative}). 
...
It gives the acceleration at which the price of a call moves along with the underlying price, ceteris paribus. It gives information on the curvature of the function to be approximated using the differential form. 
It can be crucial, at least, to assess how big is the value of gamma in order to adequately hedge a position in a call. 
Indeed, if gamma is low, the rebalancing of the hedge does not have to occur as frequently as if it is high. 
It gives as information the frequency needed in order to lower the error due to too large price movement.

As a portfolio can be delta-neutral, it can also be gamma-neutral. However, making a portfolio gamma-neutral obligates 

Consequently, by gathering gamma and delta both together, one gets a more precise information on hedging against the only stock price movement is achieved.
