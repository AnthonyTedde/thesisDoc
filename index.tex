...
\begin{figure}[h]
  ...
  \floatfoot{\textbf{Notes.} The above blue density curve is constructed over the historical data of the Apple share of stock price evolution from 18th May 2017 to 18th May 2018. while the red curve is constructed from time-series generated by the function \textit{bsm\_ts} taking $\left\{ \tau = 1.0931, \alpha = 0.48229, \sigma = 0.1958  \right\}$ as parameters.
  }
...
\begin{figure}[h]
  ...
  \floatfoot{\textbf{Notes.} A REFAIRE
  }
...
\begin{figure}[ht]
  ...
  \floatfoot{\textbf{Notes.} Simulation of one hundred geometric Brownian motions by using the function \textit{bsm\_ts} of the R package \citet{randomwalk}, with the following arguments $\left\{ \tau = 1.0931, \alpha = 0.48229, \sigma = 0.1958  \right\}$.
  }
...
\Cref{p:analysis:gbm:pl:better} shows the distribution of the above-mentioned P\&Ls with a portfolio balancing applied either daily (blue curve), twice (green curve) a day or once a week (red curve).
...
\Cref{t:analysis:bsm:pl:worst} lists all the P\&Ls' results of those hedging scenarios.
...
Nonetheless, the only information provided by \cref{t:analysis:bsm:pl} is the mean of all the calculated P\&Ls and therefore lacks in exhaustiveness.
...
Latter are depicted in \cref{p:analysis:mjd:pl:dist:big} where the green, blue and red-filled density curves respectively denote the P\&Ls' distributions when the delta-neutral portfolios are balanced twice a day, daily or weekly.
...
However, with high growth time-series as described in \cref{p:analysis:mjd:100}, a collateral effect is that they rapidly affect such out-of-the-money options prices, just as the values of the delta are influenced with a higher rate of change.
...
\Cref{p:analysis:mjd:pl:dist:deltas} shows the distribution of the P\&Ls when the hedging is built either with BSM or MJD deltas. The red density curve concerns the data related to BSM deltas while the green represents the distributions of the performance's metrics for the MJD delta-hedging.
...
According to the results of \cref{t:analysis:merton:pl} and \cref{p:analysis:mjd:pl:dist:deltas}, hedging in real life with the BSM-related delta seems not so wrong regarding the gotten measures.
Let us dive more deeply into those delta-hedging strategies by observing what happens behind the hood.

\Cref{p:analysis:mjd:hedge:deltas} illustrates how the delta-hedging works depending on the strike price and computed delta.
...
In \Cref{t:analysis:heston:pl}, the information are completely organized the same way that in \cref{t:analysis:heston:pl}. 
