...
\floatfoot{
The above stochastic processes have been constructed following \crefrange{eq:other:hsvvol}{eq:rhomerton}. The Brownian motion correlation is set to $\rho = -1$. The other parameters are set with the following values; $S(0) = 50$,  $V(0) = 0.2$,  $T = 5$ (years, along with a time step of 365 per year),  $\alpha = 0$,  $\kappa = 0.5$,  $\Theta = 0.2$,  $\sigma = 0.1$. 
} 
